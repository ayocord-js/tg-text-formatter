...
describe("TelegramMentionParser", () => {
    ...
    it ("should returns array of mentions", () => {
        expect(TelegramMentionParsers.mentions("@Stickers @akakuke", true)).toStrictEqual(["@Stickers", "@akakuke"])
        expect(TelegramMentionParsers.mentions("@Stickers @@akakuke", true)).toStrictEqual(["@Stickers"])
        expect(TelegramMentionParsers.mentions("Stickers akakuke", true)).toStrictEqual([])
        expect(TelegramMentionParsers.mentions("@Stickers @@akakuke", false)).toStrictEqual(["Stickers"])
    })
